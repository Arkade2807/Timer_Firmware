...
joystick_position_t joystick_get_pos(void)
{
    ...
    if (x_pos > JOYSTICK_THRESH_X_LEFT_2P7V && y_pos > JOYSTICK_THRESH_Y_UP_2P7V){
        return JOYSTICK_POS_UPPER_LEFT;
    }
    // If the joystick is left and down
    else if (x_pos > JOYSTICK_THRESH_X_LEFT_2P7V && y_pos < JOYSTICK_THRESH_Y_DOWN_0P6V){
        return JOYSTICK_POS_LOWER_LEFT;
    }
    // If the joystick is just left
    else if (x_pos > JOYSTICK_THRESH_X_LEFT_2P7V){
        return JOYSTICK_POS_LEFT;
    }
    // If the joystick is right and up
    else if (x_pos < JOYSTICK_THRESH_X_RIGHT_0P6V && y_pos > JOYSTICK_THRESH_Y_UP_2P7V){
        return JOYSTICK_POS_UPPER_RIGHT;
    }
    // If the joystick is right and down
    else if (x_pos < JOYSTICK_THRESH_X_RIGHT_0P6V && y_pos < JOYSTICK_THRESH_Y_DOWN_0P6V){
        return JOYSTICK_POS_LOWER_RIGHT;
    }
    // If the joystick is just right
    else if (x_pos < JOYSTICK_THRESH_X_RIGHT_0P6V){
        return JOYSTICK_POS_RIGHT;
    }
    // If the joystick is just up
    else if (y_pos > JOYSTICK_THRESH_Y_UP_2P7V){
        return JOYSTICK_POS_UP;
    }
    else if (y_pos < JOYSTICK_THRESH_Y_DOWN_0P6V){
